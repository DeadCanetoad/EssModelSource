...
uses QForms, SysUtils, QGraphics,
  uConfig, Classes, uIterators, QDialogs, uViewIntegrator{, gifimage};
{$endif}
...
begin
  try
    ...
    ShowMessage('MS XML 3 or later is required.'#13#10'Download from http://msdn.microsoft.com/xml');
    Abort;
  end;
  Result.async := false;
end;
...
begin
  ...
  Detail := MakeDOM;
  Detail.loadXML( Config.GetResourceText('p_detail_xsl_file') );
end;
