...
begin
  //Tvingar att listeners släpps samt att lagring av diagram sker
  Panel.Hide;
  Package := nil;
  ClearDiagram;
...
begin
  ...
  begin
    ...
    //Om det endast finns ett paket (förutom unknown) så visa den
    //Tilldela med Package-property så att lyssnare anropas
  ...
  begin
    //Logic package
    //Exkludera unknown package, annars så kommer alla skräpklasser med vi ShowAllClasses
    //Samt att unknown-paketet visas vid package overview (även vid dokgen)
    if IsAllClasses then
    begin
      //Koden nedan visar alla klasser i alla paket på ett diagram
  ...
  //Läs in layout för detta diagram
  FetchCount := FetchDiagram;

  //Skapa pilar mellan lådor
  //Detta måste ske efter fetchdiagram eftersom Connection-setting kan ligga lagrad
  ResolveAssociations;
...
//Lägg till en 'box' i diagram, d.v.s. en klass/interface/package
...
begin
  ...
  //Klass
  begin
    //Lägg ut relaterade boxar ifrån andra paket
    //Detta skall ej ske ifall IsAllClasses därför att då placeras alla boxar ut ändå
    if not IsAllClasses then
    begin
      //Ancestor som ligger i annat paket och som inte redan
      //är utplacerad läggs till i diagrammet
      C := (E as TClass);
      ...
      //är utplacerad läggs till i digrammet
      Mi := C.GetImplements;
      while Mi.HasNext do
      begin
      ...
      //Attributes associations som ligger i andra paket läggs till
      if ShowAssoc then
      begin
        Mi := C.GetAttributes;
        while Mi.HasNext do
        begin
          ...
          if Assigned(A.TypeClassifier) and (GetBox(A.TypeClassifier.FullName)=nil) and
            ...
            (A.TypeClassifier.Owner<>Model.UnknownPackage) then //Undvik att få in skräp ifrån unknown (java 'int' tex)
          begin
  ...
  for I := 0 to BoxNames.Count - 1 do
    ...
    begin //Class
      ...
      begin
        ...
        begin
          A := TAttribute(Mi.Next);
          //Undvik pilar som pekar på sig själv, samt assoc mot ancestor (dubbla pilar)
          ...
          begin
            ...
            //Test sker att det är samma entity, detta filtrerar bort TDatatype som kan ha samma namn som klass
  ...
  if Assigned(Ini) then
    try
      ...
      begin
        ...
        begin
          //De som inte finns lagrade får default uppe i vänstra hörnet
