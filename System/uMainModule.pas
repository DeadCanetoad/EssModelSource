...
uses
  ...
  {$IFDEF DRAG_SUPPORT}DropSource, DropTarget, {$ENDIF}Menus, uFeedback, uTreeViewIntegrator;
{$endif}
...
var
  Xmi : TXmiExporter;
begin
  Xmi := TXmiExporter.Create(Model,Feedback);
  try
    Screen.Cursor := crHourGlass;
    Xmi.InitFromModel;
    if DestFile='' then
...
begin
  ...
  begin
    ...
    D.Filter := 'GIF files (*.gif)|*.gif|WMF files (*.wmf)|*.wmf|All files (*.*)|*.*';
  ...
  begin
    ...
    begin
      if D.FilterIndex=1 then
        D.FileName := ChangeFileExt(D.FileName,'.gif')
